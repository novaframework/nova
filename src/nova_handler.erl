...
execute(Req, Env = #{module := Module, function := Function}) ->
    ...
    try
        RetObj = erlang:apply(Module, Function, [Req]),
    ...
    case proplists:get_value(fallback_controller, Attributes, undefined) of
        undefined ->
            ...
            Payload = #{
                ...
                extra_msg => list_to_binary(
                    io_lib:format(
                        "Controller returned unsupported data: ~p",
                        [Response]
                    )
                )
            },
            ...
            case function_exported(FallbackModule, resolve, 2) of
                ...
                _ ->
                    Payload = #{
                        ...
                        extra_msg => list_to_binary(
                            io_lib:format(
                                "Fallback controller ~s does not have " ++
    ...
    case nova_handlers:get_handler(element(1, RetObj)) of
        ...
        {error, not_found} ->
            case IsFallbackController of
                true ->
                    ?LOG_ERROR(#{
                        msg => <<"Controller returned unsupported result">>,
                        controller => Module,
                        function => Function,
                        return => RetObj
                    }),
...
-spec render_response(Req :: cowboy_req:req(), Env :: map()) ->
    {ok, Req :: cowboy_req:req(), State :: map()}.
        ...
        true ->
            case nova_router:lookup_url(StatusCode) of
                {error, _} ->
                    ...
                    {ok, Req0} = nova_basic_handler:handle_status(
                        {status, StatusCode}, {dummy, dummy}, Req
                    ),
                    render_response(Req0, Env);
