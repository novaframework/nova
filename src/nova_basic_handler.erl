...
-type controller() :: HandlerCallback :: function() | undefined.
...
-spec handle_json({json, JSON :: map()} | {json, StatusCode :: integer(), Headers :: map(), JSON :: map()},
                  Controller :: controller(), Req :: cowboy_req:req()) -> {ok, State :: cowboy_req:req()} |
                                                                   {Module :: atom(), Req :: cowboy_req:req()}.
handle_json({json, StatusCode, Headers, JSON}, _Controller, Req) ->
    ...
    {ok, Req2};
handle_json({json, JSON}, Controller, Req = #{method := Method}) ->
    case Method of
        <<"POST">> ->
            handle_json({json, 201, #{}, JSON}, Controller, Req);
        _ ->
            handle_json({json, 200, #{}, JSON}, Controller, Req)
    end.
...
-spec handle_ok({ok, Variables :: erlydtl_vars()} | {ok, Variables :: erlydtl_vars(), Options :: map()},
                Controller :: controller(), Req :: cowboy_req:req()) -> {ok, cowboy_req:req()}.
handle_ok({ok, Variables}, Controller, Req) ->
    ControllerInfo = erlang:fun_info(Controller),
    Mod = proplists:get_value(module, ControllerInfo),
    %% Derive the view from module
    ViewNameAtom = get_view_name(Mod),
    handle_view(ViewNameAtom, Variables, #{}, Req);
handle_ok({ok, Variables, Options}, Controller, Req) ->
    ControllerInfo = erlang:fun_info(Controller),
    Mod = proplists:get_value(module, ControllerInfo),
    View =
...
%% @end
%%--------------------------------------------------------------------
handle_view({view, Variables}, Controller, Req) ->
    handle_ok({ok, Variables}, Controller, Req);
handle_view({view, Variables, Options}, Controller, Req) ->
    handle_ok({ok, Variables, Options}, Controller, Req).
...
-spec handle_status({status, StatusCode :: integer()} |
                    ...
                    Controller :: controller(), Req :: cowboy_req:req()) -> {ok, Req :: cowboy_req:req()}.
handle_status({status, Status, ExtraHeaders, JSON}, _Controller, Req) when is_map(JSON) ->
    ...
    {ok, Req2};
handle_status({status, Status, ExtraHeaders, Body}, _Controller, Req) ->
    ...
    {ok, Req2};
handle_status({status, Status, ExtraHeaders}, _Controller, Req) ->
    Req0 = cowboy_req:set_resp_headers(ExtraHeaders, Req),
    Req1 = Req0#{resp_status_code => Status},
    {ok, Req2, _Env} =  nova_router:render_status_page(Status, #{}, Req1),
    {ok, Req2};
handle_status({status, Status}, Controller, State) when is_integer(Status) ->
    handle_status({status, Status, #{}}, Controller, State).
...
%% @end
%%-----------------------------------------------------------------
-spec handle_redirect({redirect, Route :: list()|binary()}, Controller :: controller(),
                      Req :: cowboy_req:req()) -> {ok, Req :: cowboy_req:req()}.
handle_redirect({redirect, Route}, Controller, Req) when is_list(Route) ->
    handle_redirect({redirect, list_to_binary(Route)}, Controller, Req);
handle_redirect({redirect, Route}, _Controller, Req) ->
...
-spec handle_sendfile({sendfile, StatusCode :: integer(), Headers :: map(), {Offset :: integer(),
                  ...
                  Controller :: controller(), Req) -> {ok, Req} when Req :: cowboy_req:req().
handle_sendfile({sendfile, StatusCode, Headers, {Offset, Length, Path}, Mime}, _Controller, Req) ->
...
%% @end
%%-----------------------------------------------------------------
-spec handle_websocket({websocket, ControllerData :: any()}, Controller :: controller(), Req :: cowboy_req:req()) ->
                              {ok, Req :: cowboy_req:req()}.
handle_websocket({websocket, ControllerData}, Controller, Req) ->
    ControllerInfo = erlang:fun_info(Controller),
    Module = proplists:get_value(module, ControllerInfo),
