...
start_cowboy(Configuration) ->
    Middlewares = [
        %% Lookup routes
        nova_router,
        %% Handle pre-request plugins
        nova_plugin_handler,
        %% Handle security
        nova_security_handler,
        %% Controller
        nova_handler,
        %% Handle post-request plugins
        nova_plugin_handler
    ],
    StreamHandlers = maps:get(stream_handlers, Configuration, [
        nova_stream_h, cowboy_compress_h, cowboy_stream_h
    ]),
    Options = maps:get(options, Configuration, #{compress => true}),

    %% Build the options map
    CowboyOptions1 = Options#{
    ...
    Dispatch =
        case BootstrapApp of
            undefined ->
                ?LOG_ERROR(#{
                    msg => <<"You need to define bootstrap_application option in configuration">>
                }),
