...
start_cowboy(Configuration) ->
    ...
    Dispatch =
        case BootstrapApp of
            ...
            App ->
                nova_router:compile(compile_order(App))
        end,
...
%% @doc The order applications are compiled in.
%%
%% nova is compiled last so that its own 404 and 500 routes act as defaults.
%% Routes are first-wins, so compiling nova first made an application's own
%% status-code routes unreachable.
-spec compile_order(App :: atom()) -> [nova_app()].
compile_order(App) ->
    ExtraApps = application:get_env(App, nova_apps, []),
    resolve_nova_apps([App | ExtraApps] ++ [nova]).
